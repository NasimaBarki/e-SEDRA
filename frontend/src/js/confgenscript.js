...
export function generaleScript(updateResultCallback) {
    function updateResult(newResult) {
        if (typeof updateResultCallback === 'function') {
            updateResultCallback(newResult); // Call the callback with the new user data
        }
    }
            ...
            fetch(apiBaseUrl + '/updruoli', {
                ...
                .then(text => {
                    ...
                    if (text.substring(0, 2) === '->') {
                        console.log('no')
                        alert('Errore 000-20: ' + text);
                        logerror('-20', 'setActRuoli.php', text);
                    } else {
                        console.log('refresh')
    ...
    document.getElementById("scPsw").addEventListener("change", function () {
        ...
        if (scPswValue == 0) {
            ggMsgPsw.value = 0;
            postRequest(apiBaseUrl + '/config', { item: 'scPsw', val: 0 });
    ...
    document.getElementById("emailNoRep").addEventListener("change", function () {
        ...
        postRequest(apiBaseUrl + '/config', { item: 'emailNoRep', val: mail });
    });
    ...
    document.querySelectorAll('.inputcfg').forEach(function (input) {
        input.addEventListener('change', function () {
            ...
            postRequest(apiBaseUrl + '/config', { item: item, val: value });
        });
    });
